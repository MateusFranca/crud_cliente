  ...
  <div class="container mt-5">
    ...
    <form @submit.prevent="adicionarVenda" class="mb-4">
      <div class="form-row">
        ...
        <div class="col-md-3 mb-3">
          <select v-model="novaVenda.cliente_id" class="form-control" required>
    ...
    <div class="table-responsive">
      <table class="table table-striped text-center">
        ...
        <tbody>
          <tr v-for="venda in vendas" :key="venda.id" class="hover-row">
            <td>{{ venda.valor }}</td>
            <td>{{ venda.nome }}</td>
            <td>{{ venda.descrição }}</td>
            <td>
              <button @click="editarVenda(venda.id)" class="btn btn-primary">Editar</button>
              <button @click="excluirVenda(venda.id)" class="btn btn-danger">Excluir</button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editarModalLabel">Editar Venda</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form @submit.prevent="salvarEdição">
...
export default {
  ...
  $('#editarModal').modal('show');
    ...
    salvarEdição() {
        axios.put(`/api/vendas/${this.vendaEmEdição.id}`, this.novaVenda)
        .then(response => {
  ...
  const confirmar = window.confirm("Tem certeza de que deseja excluir esta venda?");
  if (confirmar) {
